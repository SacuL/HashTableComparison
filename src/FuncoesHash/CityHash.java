 ...
 *
 */
public class CityHash implements InterfaceHashing {
    ...
    public long hashLong(String s) {
        ...
        if (len <= 32) {
            if (len <= 16) {
                ...
                return hashLen0to16(b, pos, len);
            } else {

                return Math.abs(hashLen17to32(b, pos, len));
            }
        } else if (len <= 64) {
            return Math.abs(hashLen33to64(b, pos, len));
        }
        System.out.println("ERRO");

        long x = fetch64(b, pos + len - 40);
        long y = fetch64(b, pos + len - 16) + fetch64(b, pos + len - 56);
        long z = hashLen16(fetch64(b, pos + len - 48) + len, fetch64(b, pos + len - 24));

        long[] v = weakHashLen32WithSeeds(b, pos + len - 64, len, z);
        long[] w = weakHashLen32WithSeeds(b, pos + len - 32, y + k1, x);
        x = x * k1 + fetch64(b, pos + 0);

        len = (len - 1) & (~63);
        do {
            x = rotate(x + y + v[0] + fetch64(b, pos + 8), 37) * k1;
            y = rotate(y + v[1] + fetch64(b, pos + 48), 42) * k1;
            x ^= w[1];
            y += v[0] + fetch64(b, pos + 40);
            z = rotate(z + w[0], 33) * k1;
            v = weakHashLen32WithSeeds(b, pos + 0, v[1] * k1, x + w[0]);
            w = weakHashLen32WithSeeds(b, pos + 32, z + w[1], y + fetch64(b, pos + 16));
            {
                long swap = z;
                z = x;
                x = swap;
